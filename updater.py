...
def update_protein_data(get_protein_data):
    protein_df = pd.read_csv('bulk_protein_data.csv')
    ...
    protein_df_updated.to_csv('bulk_protein_data.csv', index=False)
...
def update_creatine_data(get_creatine_data):
    ...
    creatine_df_updated.to_csv('bulk_creatine_data.csv', index=False)
